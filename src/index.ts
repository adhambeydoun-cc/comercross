...
fastify.post('/webhook/dialpad', async (request: FastifyRequest, reply: FastifyReply) => {
  try {
    ...
    fastify.log.info('🔍 DEBUG: Payload structure: ' + JSON.stringify({
      ...
      hasDateEnded: !!payload.date_ended,
    ...
    // Accept all valid call end states: recording, connected, missed, no_answer, busy, voicemail
    const validEndStates = ['recording', 'connected', 'missed', 'no_answer', 'busy', 'voicemail', 'failed'];
    if (payload.call_id && payload.date_ended && validEndStates.includes(payload.state)) {
    ...
    } else if (payload.call_id && payload.state && !payload.date_ended) {
      // Call is in progress but not finished - skip processing to avoid duplicates
      fastify.log.info(`⏳ Skipping call ${payload.call_id} - state: ${payload.state}, not yet ended`);
      return reply.send({
        success: true,
    ...
    } else if (payload.call_id && payload.state && payload.date_ended && !validEndStates.includes(payload.state)) {
