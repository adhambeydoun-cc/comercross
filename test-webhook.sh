...
# Test Script for Dialpad to BuilderPrime Webhook Integration
# This script helps test the webhook with realistic data
...
# Test 2: Generic Webhook (No Auth)
echo "📋 Test 2: Generic Webhook (No Auth)"
echo "-----------------------------------"
curl -s -X POST http://localhost:3000/webhook/generic \
  -H "Content-Type: application/json" \
  -d '{"test": "generic webhook data", "timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)'"}' | \
  jq '.' 2>/dev/null || curl -s -X POST http://localhost:3000/webhook/generic \
  -H "Content-Type: application/json" \
...
curl -s -X POST http://localhost:3000/webhook/dialpad \
    ...
    "events": [
      {
        ...
        "data": {
          ...
          "end_time": "'$(date -u -d '+1 minute' +%Y-%m-%dT%H:%M:%S.%3NZ)'",
          "duration": 60,
          "status": "answered",
  ...
  }' | jq '.' 2>/dev/null || curl -s -X POST http://localhost:3000/webhook/dialpad \
  -H "Content-Type: application/json" \
  -d '{"events": [{"event_type": "call_log.created", "data": {"call_id": "test123", "direction": "inbound", "from_number": "+15551234567", "to_number": "+15559876543", "start_time": "'$(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)'", "end_time": "'$(date -u -d '+1 minute' +%Y-%m-%dT%H:%M:%S.%3NZ)'", "duration": 60, "status": "answered"}}]}'
echo ""
...
curl -s -X POST http://localhost:3000/webhook/dialpad \
    ...
    "events": [
      {
        ...
        "data": {
          ...
          "start_time": "'$(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)'",
          "end_time": "'$(date -u -d '+2 minutes' +%Y-%m-%dT%H:%M:%S.%3NZ)'",
          "duration": 120,
          "status": "answered",
  ...
  -H "Content-Type: application/json" \
  -d '{"events": [{"event_type": "call_log.created", "data": {"call_id": "test456", "direction": "outbound", "from_number": "+15559876543", "to_number": "+15551234567", "start_time": "'$(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)'", "end_time": "'$(date -u -d '+2 minutes' +%Y-%m-%dT%H:%M:%S.%3NZ)'", "duration": 120, "status": "answered"}}]}'
echo ""
...
curl -s -X POST http://localhost:3000/webhook/dialpad \
  ...
  -H "Content-Type: application/json" \
  -d '{"events": [{"event_type": "call_log.created", "data": {"call_id": "test789", "direction": "inbound", "from_number": "+15555555555", "to_number": "+15559876543", "start_time": "'$(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)'", "end_time": "'$(date -u +%Y-%m-%dT%H:%M:%S.%3NZ)'", "duration": 0, "status": "missed"}}]}'
echo ""

echo "🎯 Test Summary"
echo "==============="
echo "✅ All webhook endpoints tested"
